...
errp <- ggplot2::ggplot(average_reaction_time) +
  ...
  geom_errorbar(aes(Days, ymin = lower_reaction_time, ymax = upper_reaction_time, colour = factor(Days)),
                size  = .4,
                show.legend = FALSE) +
    labs(title = "Mean evolution with errorbars",
       x       = "Days of sleep deprivation",
       y       =  NULL) +
  scale_x_continuous(breaks = 0:9)
...
Looking at the correlation matrix, there is a correlation higher than 0.6 between subsequent days (e.g. between Day 3 and 4, between Day 8 and 9, ...). However, the further the days are apart, the lower the correlation (e.g. low correlation between Day 1 and Day 8). 
...
With the level 2 model we are trying to explain the variation between individuals using the intercept and at the slope while
...
The model is built on the assumption that the b's come from a normal distribution with mean 0. The residual variance in true intercept $\pi_{0i}$ across all individuals in the population is 611.9, the residual variance in true slope $\pi_{1i}$ across all individuals in the population is 35.08 and the residual covariance between the true intercept $\pi_{0i}$ and the slope $\pi_{1i}$ is 9.61.


Next step is to check if the values we retrieved are actually significant. We therefore check if the number of days have a significant effect on the reaction time.
We tested the fixed effects with Bootstrap and profile likelihood as the sample size was too small to use a Wald test. We checked different possible models using a likelihood ratio test.
...
pander::pander(confint(sleep_reml,
        ...
        oldNames  = F))
```
The confidence intervals of the intercept and Days do not include 0. Therefore, both of them have a significant effect on the Reaction time.
...
```
We compared an intercept-only model with a model that includes Days to find the best model using MLE. Looking at the outcome of the  likelihood ratio test with Anova, we can conclude that adding Days as covariate improves our model significantly as it has a significant effect on the Reaction time with a p-value much smaller than 0.05.
