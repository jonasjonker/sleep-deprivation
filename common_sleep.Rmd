...
head(sleep, 12)
```
This dataset contains multiple measurements for each subject on consecutive days, with as response variable the continuous variable reaction time (= 'Reaction') and explanatory variable 'Days'. Since there are 10 measurements for each subject, it is a longitudinal study. The dataset of 18 subjects is balanced and complete (i.e. there is no missing data) with an equal amount of measurements for each subject.

### Spaghetti Plot
To visualise the individual reaction times and how they compare to the mean, a spaghetti plot was created. This revealed that there was a variation in intercepts or starting reaction times on day 0 between subjects. This variation between subjects increased with subsequent days. 
...
sleep_spag_plot <- ggplot(sleep) +
  ...
  labs(title   = "Spaghetti Plot",
       caption = "Reaction time seems to increase with each day of sleep deprivation",
...
The mean and median seem to show a similar increase throughout the study. For day nr 6, 7 and 10, outliers are observed. We can observe that the variance increases with an increase in days of sleep deprivation but the interquartile range appears to expand not as strongly as the minimum and maximum of the boxplot. 
...
```
The calculations of the mean, standard deviation and variance of the reaction time\nfor each day of all subjects, further support our exploratory plots: we observe an increase in the mean\n, variance, and standard deviation with more days of sleep deprivation. (note that on day 2 and 7 the variance and hence standard deviation slightly decrease compared to the day before but rise again after.)
...
ggplot2::ggplot(average_reaction_time) +
  ...
  scale_x_continuous(breaks = 0:9)
```
To further support our previous findings, we looked at the mean evolution. Here, a trend of increasing reaction time with increasing number of days is also observed, together with expanding standard deviations (see errorbars).
...
ggplot(melt(cors), aes(Var1,Var2, fill = value)) +
  ...
  labs(title = "Correlation between reaction time and days",
       x = "reaction on day #",
       y = "reaction on day #")
```
...
Looking at the correlation matrix, there is a correlation higher then 0.6 between subsequent days (e.g. between Day 3 and 4, between Day 8 and 9, ...). However, the further the days are apart, the lower the correlation (e.g. low correlation between Day 1 and Day 8). 


Aligning nicely with our previous results, we can conclude that there is a linear trend between the number of days and reaction time.
