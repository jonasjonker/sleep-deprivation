...
--> what is meant by variation? shouldnt we define it as the difference between the minimum and maximum??
...
--> should we add anything more to this??
...
Aligning nicely with our previous results, there is a linear trend between the number of Days and reaction time. --> isn't this another point? 
...
```
We performed linear regression models on each subject's 
We made a trellis graph to visualise the intercepts and slopes from the subject-specific linear regression models.   

The graph suggests that the slope and intercept of one subject's linear model are independent of each other, due to no observable trend between increasing slope and intercept. Overall, all subjects have a positive slope besides subject 335. 
...
The individual intercepts correspond to the initial reaction time and are non normally distributed. Given the small data set, this is not surprising as it shows a variety of their initial reaction time. However, if this data came from a large dataset, it would be surprising that even the initial data points are not normally distributed and could suggest a wrong data sample compared to the population. 
